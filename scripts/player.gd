...
func _physics_process(delta: float) -> void:
	...
	if Input.is_action_just_pressed("drop") and Global.held_weights != []:
		...
		instance.editor_description = str(weight_to_spawn) # Set the weight
		instance.position = position + Vector2(0,-15) # Set position to player's position (plus a bit above their head)
...
func update_debug_texts(): # Debug
	$"camera/CanvasLayer/Control/VBoxContainer/held weights".text = str("held weights: ",Global.held_weights)
